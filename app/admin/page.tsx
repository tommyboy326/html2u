...
export default async function AdminPage({
  ...
  if (!(await isAdmin())) {
    return (
      <main className="centered">
        ...
        >
          <h1 style={{ fontSize: 28, marginBottom: 4 }}>🛡 管理後台</h1>
          {HAS_GOOGLE_AUTH ? (
            <form action={loginWithGoogle} className="card unlock stack">
              <p className="muted">僅限授權的 Google 帳號可進入。</p>
              {sp.error && (
                <p className="error">此 Google 帳號未獲授權,或登入失敗。</p>
              )}
  ...
  return (
    <main className="container wide">
      <div className="rowbetween" style={{ marginBottom: 32 }}>
        <h1 className="display-lg">管理後台</h1>
      ...
      <form className="row" style={{ marginBottom: 24 }}>
        ...
        <button className="utility" type="submit">搜尋</button>
      </form>
          ...
          <tbody>
            ...
            {items.map((s) => (
              <tr key={s.id} className={s.reports > 0 ? "flagged" : undefined}>
                ...
                <td className="small">{fmt(s.expiresAt)}</td>
                <td>
